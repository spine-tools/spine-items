...
class TestCheckableFileListModel(unittest.TestCase):
    ...
    def test_update(self):
        ...
        self.assertEqual(index.data(Qt.ToolTipRole), str(Path.cwd() / "path" / "to" / "file"))
        self.assertEqual(self._model.rowCount(index), 0)
        index = self._model.index(1, 0)
        self.assertEqual(index.data(), "pack label")
        self.assertEqual(index.data(Qt.ToolTipRole), None)
        ...
        self.assertEqual(index.data(Qt.ToolTipRole), str(Path.cwd() / "path" / "to" / "pack_file"))
    ...
    def test_replace_single_resource(self):
        ...
        self.assertEqual(index.data(Qt.ToolTipRole), str(Path.cwd() / "path" / "to" / "another_file"))
        self.assertEqual(self._model.rowCount(index), 0)
    ...
    def test_replace_pack_resource_file_path(self):
        ...
        self.assertEqual(index.data(Qt.ToolTipRole), str(Path.cwd() / "path" / "to" / "pack_file_2"))
