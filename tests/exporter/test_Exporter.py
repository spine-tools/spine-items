...
class TestExporter(unittest.TestCase):
    ...
    def test_rename(self):
        ...
        self.assertEqual(
            expected_name, self.exporter._properties_ui.item_name_label.text()
        )  # name label in props
        self.assertEqual(
            expected_name, self.exporter.get_icon().name_item.text()
    ...
    def test_activation_populates_properties_tab(self):
        ...
        for i in range(self.exporter._properties_ui.databases_list_layout.count()):
            database_item = self.exporter._properties_ui.databases_list_layout.itemAt(
                i
            ).widget()
    ...
    def test_activating_second_exporter_with_less_database_urls_does_not_crash(self):
        ...
        self.assertTrue("second url to database" in urls_in_properties_tab)
        resources = [
            ProjectItemResource(None, "database", "url to a database in the clouds")
        ]
        ...
        self.assertEqual(
            database_item.url_field.text(), "url to a database in the clouds"
        )

    def test_handle_dag_changed_does_not_overwrite_properties_tab_when_no_urls_change(
        self,
    ):
        self.exporter._start_worker = MagicMock()
        self.exporter.activate()
