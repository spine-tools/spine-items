...
from spine_engine.helpers_qt_free import shorten, create_log_file_timestamp
...
class ExecutableItem(ExecutableItemBase):
    ...
    def _execute_forward(self, resources):
        ...
        if not to_urls:
            ...
            return True
        from_db_maps = [db_map for db_map in (self._get_db_map(url) for url in from_urls) if db_map]
        to_db_maps = [db_map for db_map in (self._get_db_map(url) for url in to_urls) if db_map]
        ...
        if all_errors:
            ...
            logfile_anchor = (
                f"<a style='color:#BB99FF;' title='{logfilepath}' href='file:///{logfilepath}'>error log</a>"
            )
